...
class SystemLauncher:
    ...
    def start_socket_servers(self):
        ...
        try:
            ...
            print(f"    [+] Domain 1 (example1.com): 端口 8080 - PID {proc1.pid}")
            
            time.sleep(1)
            
            # Domain 2 - 修改配置
            import json
            with open("config/domain2_config.json", 'r', encoding='utf-8') as f:
                config = json.load(f)
            
            print(f"    [+] Domain 2 (example2.com): 端口 8081 - 运行在主进程")
            print("    [!] Socket双服务器已在同一进程中启动")
    ...
    def start_grpc_servers(self):
        ...
        try:
            ...
            if not grpc_pb2.exists():
                ...
                subprocess.run([sys.executable, "generate_grpc.py"], check=True)
                os.chdir(self.base_dir)
                print("    [+] gRPC代码生成完成")
    ...
    def show_status(self):
        ...
        # 检查端口
        print("\n[+] 端口状态:")
        ports = [8080, 8081, 50051, 50052, 8443, 8444]
        for port in ports:
            ...
            status = "[OCCUPIED]" if f":{port}" in result.stdout else "[FREE]"
            print(f"    {status} :{port}")
